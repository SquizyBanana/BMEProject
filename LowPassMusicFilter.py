...
class Music_Filter:
    def __init__(self):
        thousand_miles_away = AudioSegment.from_file("MusicFiles/10,000 Miles Away.wav", format="wav")
        hum = AudioSegment.from_file("MusicFiles/hum.wav", format="wav")
        unaltered_song = AudioSegment.from_file("MusicFiles/Hollaback Girl.wav", format="wav")
        ...
        self.thread_is_not_alive = True
        self.song_not_finished = True
        self.hum_array = []
        self.attenuation_array = [True, False]

    def play_song(self):
        while self.song_not_finished:
            # get boolean of attenuation VVVV did it now with an array
            attenuation = self.attenuation_array[random.randint(0, 2)]
            if attenuation:
                if len(self.hum_array) == 0:
                    self.hum_array.append(threading.Thread(target=play, args=(self.hum[1000:6000],)))
                    # t2 = threading.Thread(target=play, args=(hum[1000:6000],))
                    # t2.start()
                    self.hum_array[0].start()
                else:
                    if not self.hum_array[0].is_alive():
                        self.hum_array.pop(0)
